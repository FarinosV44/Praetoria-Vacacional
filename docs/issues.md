...
| #78 | ✅ code — `lighthouserc.json` (budgets + CWV thresholds) + a `lighthouse` job in `.github/workflows/ci.yml` (`@lhci/cli autorun` over a prod build + `next start`, 5 URLs × 3 runs, median). A11y / SEO / CLS are hard gates (fail CI); perf / LCP / TBT / bundle-size are `warn` annotations (CI Lighthouse is noisy on shared runners). Reports upload to temporary public storage. `docs/perf-budget.md` documents the table + how to point it at the deployed URL for real field CWV. D-042. **Owner:** after deploy, check Search Console → Core Web Vitals and run PageSpeed Insights on the live domain. |
...
| # | Title | Owner input needed |
|---|---|---|
| #67 | ✅ code — user decision: **tiered by lead time** (100% ≥30 d · 50% 7–29 d · 0% <7 d), applied to both property `cancellationPolicy` blocks. Pure `computeRefund` (6 tests). `cancelWithRefund(reservationId, {reason, overrideRefundCents?})` — policy amount or an override, issues the Stripe refund on the captured PaymentIntent (idempotency-keyed `refund_<id>`), records it on the payment row, sets `paymentState` refunded/partial, retires guest messages, emails the guest. Reconciliation: webhook `charge.refunded` handler + `reconcileStripe()` (walks recent PaymentIntents carrying our metadata, fixes drift from dashboard refunds / downtime) wired to `/api/cron/reconcile` (every 6 h, CRON_SECRET). `/admin/reservas/[id]` → `CancelWithRefund` shows the computed refund + override before confirming. 2 repo methods (`getPaymentByIntent` + parity). D-035. **Owner:** set Stripe live keys; add the `charge.refunded` event to the webhook + the reconcile cron. |
| #68 | ✅ code — `src/domains/portal/*`. Stateless magic-link token (`signPortalToken`/`verifyPortalToken` — HMAC `id.expiry.mac`, 7-day TTL, 4 tests; no DB row). `/mi-reserva` — request a link with localizador + email (rate-limited 5/10 min, no enumeration: always "if it matches…"). `/mi-reserva/[token]` — stay summary, payment status, **pay the outstanding balance** (Stripe Checkout for `total − paid`), download issued invoices, submit arrival time + requests (appended to the reservation notes + owner email). `/mi-reserva/[token]/factura/[id]` reuses the extracted `<InvoiceDocument>` (also now backs the admin viewer), token-guarded to the guest's own reservation. `noindex` (middleware) + footer link + link in the confirmation email. No migration. D-037. **Owner:** none — works on the existing Stripe/Resend setup; balance payments need Stripe live. |
...
| #70 | ✅ code (with #71) — one board covers both. |
| #71 | ✅ code — `operations_tasks` table (migration `20260902150000`): kind turnover/cleaning/maintenance/incident, status, priority, due date, assignee, cost (maintenance), photos (paste media-library URLs). Pure `planTurnovers` (auto turnover per confirmed checkout in a 45-day window; same-day back-to-back → urgent) + `compareTasks` board sort — 4 tests. 6 repo methods incl. `reconcileTurnovers` (memory + supabase, unique on reservation_id). `/admin/operaciones` — filter by kind, quick status change, expand to edit due/assignee/cost/photos, delete, "generar cambios de huésped". `/api/cron/turnovers` (daily, CRON_SECRET). New `operations.write` capability (admin + gestión). D-038. **Owner:** apply the migration; add the daily cron. |
| #72 | ✅ code — `traveller_registrations` table (migration `20260902160000`, RD 933/2021 fields). Pure `validateTraveller` (DNI/NIE format, payment method, ES-resident municipality/province), `isMinor`, `buildParte` (establishment + stay + traveller list), `registryStatus` — 8 tests. Guest **check-in online** at `/mi-reserva/[token]/checkin` (add/remove travellers, linked from the portal + confirmation email). `/admin/registro-viajeros` (next 14 days, completeness, "marcar enviado") + a panel on the reservation detail (download parte JSON, submit, mark sent). `submitParte` gated on `SES_HOSPEDAJES_*` — the SOAP binding is a documented TODO (needs the owner's WSDL/cert); manual upload + "marcar enviado" is the path until then. 6 repo methods (memory + supabase, parity). `config-status` "traveller_registry". D-041. **Owner:** register the establishment on the SES.HOSPEDAJES portal, set `SES_HOSPEDAJES_*`; until the web-service binding is finished, upload the generated parte manually. |
| #73 | ✅ code — campaigns now **actually send over Resend** (same account as transactional email — no separate provider). `sendCampaign` re-checks the suppression list per recipient, wraps the body in the branded template, attaches RFC-8058 `List-Unsubscribe` + `List-Unsubscribe-Post: One-Click` headers and a visible link, marks each recipient sent/failed, closes the campaign. Stateless HMAC unsubscribe token (`signUnsubToken`/`verifyUnsubToken`, 2 tests) → `/baja` (human confirm) + `/api/marketing/unsubscribe` (one-click POST). `/api/webhooks/resend` (Svix signature) adds bounced/complained addresses to the suppression list. `markCampaignSent` stays as the intent-only fallback for WhatsApp. `MARKETING_FROM` optional. `config-status` "campaigns" → configured when Resend is. 2 repo methods (memory + supabase, parity). D-039. **Owner:** add the Resend webhook (`/api/webhooks/resend`) + `RESEND_WEBHOOK_SECRET`; optionally a `MARKETING_FROM`. |
| #74 | ✅ code — user decision: **auto-apply within guardrails**. Pure `suggestNightlyRate` (7 tests) — named factors (última hora −12/−6%, reserva anticipada +4%, demanda ±8–18% from window occupancy, noche suelta −15%) applied to the *natural* price (static seasons/weekend config, so re-runs don't ratchet), then hard-clamped to ±`bandPct` (default 25) and never below `floorCents` (default 60% of base). Per-property settings in `content_overrides` (`enabled`/`floor`/`band`/`horizon`); `enabled` defaults **off** — turning it on is what makes the daily `/api/cron/pricing` write the rates without a click. `/admin/precios-dinamicos` shows every changed date with its natural price, recommendation, % change, clamp reason and the factor breakdown, plus "aplicar ahora". Audited. `settings.write`. D-040. **Owner:** review the factor weights + floor per property, then tick "aplicar automáticamente"; add the daily cron. |
...
| #81 | ✅ code — `media_assets` table (migration `20260902140000`) + Supabase Storage **private** bucket `media`. `src/domains/media/*`: pure `isAllowedUpload` (mime allow-list + 15 MB cap) / `focalToObjectPosition` / `extensionFor` (5 tests). `uploadMediaAction` validates → uploads to Storage (service key) → creates the row; the client reads `naturalWidth/Height` before upload. 5 repo methods (memory + supabase); `listMedia` mints batch signed URLs. `/admin/media` — upload, grid, per-asset ALT + click-to-set focal point + tags + copy signed URL + delete. `config-status` "media" feature. DEMO → upload disabled with a clear message. D-036. **Owner:** create the private `media` bucket in Supabase Storage. |
...
| #85 | 🟡 code — user decision: **draft EN, marked for review**. Infrastructure: content models take an optional `en?` block; the `/en/...` route renders it with a "draft, pending review" banner, `noindex`, out of sitemap, no hreflang. Shipped: EN drafts of the **2 destination guide hubs** (`/en/guias`, `/en/guias/javalambre`, `/en/guias/valencia-playa`) — full translations in `hubs.ts`. Home / property pages / checkout were already EN (#29). `docs/i18n-status.md` inventories what's done vs pending (satellite guides, landings, seasonal, blog, legal) with the exact pattern + the "when the owner approves a draft" checklist. D-044. **Owner:** review the guide-hub drafts; commission a professional EN legal translation (don't machine-translate legal); then flip approved pages per the doc. |
