...
| 56-L | Final E2E + docs — `chain.test.ts` drives the whole `reserva → cliente → factura → documento → calendario → historial → segmento` chain against the repository (134 unit total). `e2e/intranet.spec.ts` asserts every intranet route + export endpoint is private (77 chromium e2e green). `docs/intranet.md`, `docs/api/INDEX.md` (intranet functions + routes + migrations), `docs/SETUP.md` (ADMIN_ROLE, Airbnb feeds, campaign-send note), `.env.example`. Merged `develop → main`. | ✅ |
